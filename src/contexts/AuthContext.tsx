...
interface AuthContextType {
  ...
  signup: (username: string, password: string, accounts: any[]) => Promise<void>
  logout: () => void
}
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const signup = async (username: string, password: string, accounts: any[]) => {
    setLoading(true)
    try {
      if (!username || !password) {
        throw new Error("Username and password are required")
  ...
  return (
    <AuthContext.Provider value={{ isLoggedIn, user, loading, login, signup, logout }}>{children}</AuthContext.Provider>
  )
}
